...
- **Wallet System**
  ...
  - Shopper wallet monitoring and stats
...
- **Refunds Management**
  ...
  - Integration with wallet balances
...
## 🎁 Referral Tracking & Earnings System

The Plas Dashboard includes a comprehensive system for tracking user referrals, managing approvals, and calculating performance-based earnings.

### 📈 Business & Financial Logic

- **Commission Rate**: Set to **0.75%** of the profit/markup for each order.
...
- **Referral Table**:
  - **Orders Count**: Real-time display of total orders generated by each referral code.
  - **Volume Monitoring**: Tracks total selling amount (RWF) for all associated orders.
  - **Earnings Display**: Shows exactly how much a referral user has earned from their orders (in RWF).
- **Intelligent Detail Views**:
  - **Status-Aware Actions**: The action button automatically switches from "Review" (for pending) to "**Details**" (for active/approved) referrals.
  - **Enhanced Details Drawer**: Expanded width (`sm:max-w-3xl`) for better data visualization.
  - **Individual Order Table**: Lists every associated order separately, showing Order ID, Selling Price, and per-order earning.
  - **Status Resilience**: Handles multiple status variations (e.g., Active, approved) to ensure data visibility.

### ⚙️ Technical Implementation

- **Data Fetching**: Uses a specialized GraphQL query `GET_REFERRAL_ORDERS` to fetch orders, items, and product price details in a single efficient request.
- **API Aggregation**: The `api/referral/route.ts` endpoint processes raw order data to calculate aggregate stats (count, volume, earnings) before sending it to the frontend.
- **Performance Optimized**: Calculations are performed on the server-side to ensure the frontend remains snappy even with high order volumes.
