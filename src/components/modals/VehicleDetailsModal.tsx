...
export const VehicleDetailsModal: React.FC<VehicleDetailsModalProps> = ({
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="sm:max-w-xl w-full p-0 flex flex-col border-none shadow-2xl bg-slate-50 dark:bg-slate-950">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <SheetDescription className="flex items-center gap-2 mt-1 flex-wrap">
                <Badge variant="outline" className="capitalize font-bold border-emerald-500/20 text-emerald-500 bg-emerald-500/5">
                  {String((vehicle as any).category || 'Vehicle')}
                </Badge>
                <span className="text-slate-400">•</span>
                <span className="font-semibold text-slate-500">{String((vehicle as any).brand || 'Standard')} {(vehicle as any).model || ''}</span>
              </SheetDescription>
            </div>
          </div>
        </SheetHeader>
          ...
          <div className="px-6 py-8 space-y-10">
            ...
            <div className="flex items-center justify-between p-6 rounded-[2rem] bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800">
              <div className="space-y-1">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Daily Rate</span>
              ...
              <div className="space-y-1 text-right">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Plate Number</span>
              ...
              <div className="aspect-[4/3] rounded-[2rem] overflow-hidden bg-white shadow-xl border-4 border-white group relative">
                ...
                <div className="absolute top-4 right-4">
                  <div className="bg-white/90 backdrop-blur-md p-3 rounded-2xl shadow-lg flex items-center gap-2">
                    ...
                    <span className="text-sm font-black text-slate-900">{String((vehicle as any).location || 'Kigali')}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-8 rounded-[2rem] bg-white dark:bg-slate-900 shadow-lg border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
                ...
                <p className="text-base leading-relaxed text-slate-600 dark:text-slate-400 font-medium italic relative z-10">
                  "{String(vehicle.description || 'No detailed description available for this vehicle. Contact the logistics partner for more information regarding performance and features.')}"
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                ...
                <span className="text-sm font-black text-slate-800 dark:text-white">{String((vehicle as any).seats || '5')}</span>
              </div>
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                <Fuel className="w-6 h-6 text-orange-500 mb-2" />
                <span className="text-[9px] uppercase font-bold text-slate-400 mb-1">Fuel</span>
                <span className="text-sm font-black text-slate-800 dark:text-white capitalize">{String((vehicle as any).fuel_type || 'Petrol')}</span>
              </div>
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                <Settings className="w-6 h-6 text-purple-500 mb-2" />
                <span className="text-[9px] uppercase font-bold text-slate-400 mb-1">Transmission</span>
                <span className="text-sm font-black text-slate-800 dark:text-white capitalize">{String((vehicle as any).transmission || 'Auto')}</span>
              </div>
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                ...
                <span className="text-sm font-black text-slate-800 dark:text-white capitalize">{(vehicle as any).drive_provided ? 'Included' : 'Self-drive'}</span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider px-2">Exterior View</span>
                ...
                <div className="space-y-2">
                  <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider px-2">Interior View</span>
                  <div className="aspect-video rounded-2xl overflow-hidden border-2 border-white dark:border-slate-800 shadow-md bg-white dark:bg-slate-900 group/img relative cursor-pointer">
            ...
            <div className="p-6 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="text-[10px] font-black uppercase text-slate-400 block">Registration</span>
                  <span className="text-sm font-bold text-slate-800 dark:text-white">Listed on {new Date(vehicle.created_at).toLocaleDateString()}</span>
                </div>
              </div>
