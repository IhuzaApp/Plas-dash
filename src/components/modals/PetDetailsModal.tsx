          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <SheetDescription className="flex items-center gap-2 mt-1 flex-wrap">
                <Badge variant="outline" className="capitalize font-bold border-primary/20 text-primary bg-primary/5">
          ...
          <div className="px-6 py-8 space-y-10">
            ...
            <div className="flex items-center justify-between p-6 rounded-[2rem] bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-1 text-right">
                <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest block">Status</span>
            ...
            <div className="space-y-4">
              ...
              <div className="p-8 rounded-[2rem] bg-white dark:bg-slate-900 shadow-lg border border-slate-100 dark:border-slate-800 relative overflow-hidden group">
                ...
                <p className="text-base leading-relaxed text-slate-600 dark:text-slate-400 font-medium italic relative z-10">
                  "{String(pet.story || 'No story provided for this pet yet. They are waiting for someone to get to know them!')}"
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                ...
                <span className="text-sm font-black text-slate-800 dark:text-white">{String(pet.age || `${pet.months}m`)}</span>
              </div>
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                <Layers className="w-6 h-6 text-blue-500 mb-2" />
                <span className="text-[9px] uppercase font-bold text-slate-400 mb-1">Gender</span>
                <span className="text-sm font-black text-slate-800 dark:text-white capitalize">{String(pet.gender || 'N/A')}</span>
              </div>
              <div className="p-5 rounded-3xl bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                <Tag className="w-6 h-6 text-orange-500 mb-2" />
                <span className="text-[9px] uppercase font-bold text-slate-400 mb-1">Color</span>
                <span className="text-sm font-black text-slate-800 dark:text-white">{String(pet.color || 'N/A')}</span>
              </div>
            </div>
              ...
              <div className="p-6 rounded-[2rem] bg-emerald-500/5 border border-emerald-500/10 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  {pet.vaccination_cert && (
                    <Button variant="link" className="text-emerald-600 font-bold p-0 h-auto underline decoration-2" asChild>
                ...
                {vaccines.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {vaccines.map((v, i) => (
                      <div key={i} className="px-4 py-2 rounded-xl bg-emerald-500 text-white text-[10px] font-black uppercase tracking-wider shadow-sm">
                        {v}
                      </div>
                    ))}
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-6 rounded-[2rem] bg-white dark:bg-slate-900 shadow-md border border-slate-100 dark:border-slate-800">
                <span className="text-[10px] font-black uppercase text-slate-400 block mb-4">Favourite Foods</span>
                {favouriteFoods.length > 0 ? (
                  ...
                  <span className="text-sm text-slate-400 font-medium italic">No special diet listed</span>
                )}
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {parentImages.map((img, idx) => (
                    <div key={idx} className="aspect-square rounded-[2rem] overflow-hidden border-4 border-white dark:border-slate-800 shadow-lg bg-white dark:bg-slate-900 hover:scale-[1.02] transition-transform cursor-pointer">
                      <img src={img} alt={`Parent ${idx + 1}`} className="w-full h-full object-cover" />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {pet.video && (
              ...
              >
                ...
                <div className="relative z-10">
                  <span className="text-[10px] font-black uppercase text-white/60 block mb-1">Media</span>
                  <span className="text-2xl font-black text-white">Watch {String(pet.name || 'Pet')} in action</span>
                </div>
              </a>
            )}
          </div>
