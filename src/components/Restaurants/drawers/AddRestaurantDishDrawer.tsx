...
const AddRestaurantDishDrawer: React.FC<AddRestaurantDishDrawerProps> = ({
  ...
  const { data: searchProductNamesData, isLoading: isSearchingProducts } =
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        toast.error(
          `File is too large (${(file.size / (1024 * 1024)).toFixed(1)}MB). Maximum limit is 10MB.`
        );
        return;
      }
    ...
    try {
      ...
      if (values.mode === 'new') {
        ...
        const result = await createDishMutation.mutateAsync({
          ...
          image:
            values.image ||
            'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?q=80&w=100&h=100&auto=format&fit=crop', // Uploaded image or default
        });
      ...
      // Generate SKU
      const selectedDishCategory = searchResults?.dishes?.find(
        (d: any) => d.id === finalDishId
      )?.category;
      const categoryStr = selectedDishCategory || form.watch('category');
      const categoryCode = (categoryStr || 'GEN')
        .toUpperCase()
        .replace(/[^A-Z]/g, '')
        .slice(0, 5);
    ...
    >
      <SheetContent className="sm:max-w-[540px] flex flex-col p-0">
        <SheetHeader className="p-6 pb-2">
          ...
          <SheetDescription>
            Select an existing item from the system or create a new one, then set pricing and
            availability for your restaurant.
          </SheetDescription>
        </SheetHeader>
            ...
            >
                      ...
                      >
                        {mode === 'new'
                          ...
                          : mode === 'product'
                            ...
                            : selectedDishName || createdDishName || 'Search for a base item...'}
                            ...
                            {searchResults?.dishes?.map((d: any) => (
                              ...
                              >
                                <Check
                                  className={cn(
                                    'mr-2 h-4 w-4',
                                    selectedDishId === d.id && mode === 'existing'
                                      ? 'opacity-100'
                                      : 'opacity-0'
                                  )}
                                />
                            ...
                            {filteredProducts.map((p: any) => (
                              ...
                              >
                                <Check
                                  className={cn(
                                    'mr-2 h-4 w-4',
                                    form.watch('product_id') === p.id && mode === 'product'
                                      ? 'opacity-100'
                                      : 'opacity-0'
                                  )}
                                />
                                <div className="flex items-center gap-3">
                                  ...
                                  <div className="flex flex-col">
                                    <span className="font-medium text-sm">
                                      Create from catalog: {p.name || 'Unnamed Product'}
                                    </span>
                ...
                {mode === 'new' && (
                  <div className="space-y-4 border p-4 rounded-md bg-muted/20 animate-in fade-in zoom-in duration-300">
                    <div className="flex items-center gap-2 mb-2 text-sm text-muted-foreground bg-primary/10 text-primary px-3 py-2 rounded-md">
                      <span>
                        Creating a new global base dish. A default placeholder image will be
                        assigned.
                      </span>
                    </div>
                    <FormField
              ...
              <div className="border-t pt-6 animate-in fade-in zoom-in duration-300">
                <h3 className="text-sm font-semibold mb-4 text-primary">
                  Restaurant Menu Pricing & Inventory
                </h3>
        ...
        <SheetFooter className="p-6 border-t bg-background">
          <Button
            ...
            onClick={onClose}
            disabled={
              addDishToMenuMutation.isPending || createDishMutation.isPending || isUploading
            }
          >
            Cancel
          </Button>
