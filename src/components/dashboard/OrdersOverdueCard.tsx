...
const DUMMY_ORDERS = [
  { id: '1', OrderID: '1001', created_at: new Date(Date.now() - 20 * 60 * 1000).toISOString(), shopper_id: null, status: 'pending', address: '123 Main St', shop: 'SuperMart' },
  { id: '2', OrderID: '1002', created_at: new Date(Date.now() - 40 * 60 * 1000).toISOString(), shopper_id: null, status: 'pending', address: '456 Oak Ave', shop: 'FreshFoods' },
  { id: '3', OrderID: '1003', created_at: new Date(Date.now() - 90 * 60 * 1000).toISOString(), shopper_id: null, status: 'pending', address: '789 Pine Rd', shop: 'MegaMarket' },
  { id: '4', OrderID: '1004', created_at: new Date(Date.now() - 16 * 60 * 1000).toISOString(), shopper_id: null, status: 'pending', address: '321 Maple St', shop: 'QuickShop' },
  { id: '5', OrderID: '1005', created_at: new Date(Date.now() - 120 * 60 * 1000).toISOString(), shopper_id: null, status: 'pending', address: '654 Elm St', shop: 'SuperMart' },
  { id: '6', OrderID: '1006', created_at: new Date(Date.now() - 30 * 60 * 1000).toISOString(), shopper_id: null, status: 'pending', address: '987 Cedar Ave', shop: 'FreshFoods' },
];
                ...
                <TableBody>
                  {overdueOrders.map((order, idx) => (
                    <TableRow key={order.id} className={idx % 2 === 0 ? 'bg-white/70' : 'bg-muted'}>
                      ...
                      <TableCell>
                        {DUMMY_MODE ? (
                          <span className="inline-block px-2 py-1 rounded bg-yellow-100 text-yellow-800 text-xs font-semibold">{order.status}</span>
                        ) : (
                          <span className="inline-block px-2 py-1 rounded bg-yellow-100 text-yellow-800 text-xs font-semibold">{order.status}</span>
                        )}
                      </TableCell>
                      <TableCell>{formatDistanceToNow(parseISO(order.created_at), { addSuffix: true })}</TableCell>
                      <TableCell>{isDummyOrder(order) ? order.address : (order.Address?.street || 'N/A')}</TableCell>
                      <TableCell>{
                        isDummyOrder(order)
                          ? order.shop
                          : ((order as any).Shop?.name || 'N/A')
                      }</TableCell>
                      <TableCell className="text-right space-x-2">
                        <Button size="sm" variant="outline" disabled>Assign</Button>
                        <Button size="sm" variant="default" disabled>Combine</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
