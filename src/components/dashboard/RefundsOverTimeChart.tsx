...
export default function RefundsOverTimeChart({ refunds, isLoading }: Props) {
    ...
    const chartData = useMemo(() => {
        ...
        const buckets: Record<string, { approved: number; pending: number; rejected: number; in_review: number }> = {};
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        {isLoading ? (
                            ...
                            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                                ...
                                <YAxis tick={{ fontSize: 11, fill: TICK_FILL }} axisLine={{ stroke: GRID_STROKE }} tickLine={{ stroke: GRID_STROKE }} allowDecimals={false} />
                                <Tooltip
                                    contentStyle={{
                                ...
                                <Area type="monotone" dataKey="Approved" stackId="1" stroke={CHART_COLORS.green} fill={CHART_COLORS.green} fillOpacity={0.65} strokeWidth={1.5} />
                                <Area type="monotone" dataKey="Pending" stackId="1" stroke={CHART_COLORS.yellow} fill={CHART_COLORS.yellow} fillOpacity={0.65} strokeWidth={1.5} />
                                <Area type="monotone" dataKey="In Review" stackId="1" stroke={CHART_COLORS.blue} fill={CHART_COLORS.blue} fillOpacity={0.65} strokeWidth={1.5} />
                                <Area type="monotone" dataKey="Rejected" stackId="1" stroke={CHART_COLORS.red} fill={CHART_COLORS.red} fillOpacity={0.65} strokeWidth={1.5} />
                            </AreaChart>
                        )}
                    </ResponsiveContainer>
                </div>
