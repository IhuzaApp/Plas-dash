        ...
        <div className="mt-6 space-y-5">
          ...
          <Card>
            ...
            <CardContent className="px-4 pb-4 divide-y divide-border">
              <Row label="Refund ID" value={<span className="font-mono text-xs">{refund.id}</span>} />
              <Row label="Order ID" value={<span className="font-mono text-xs">{refund.order_id}</span>} />
              <Row label="Payment"
                value={
              ...
              <Row label="Created" value={format(new Date(refund.created_at), 'MMM dd, yyyy HH:mm')} />
              {refund.update_on && (
                <Row label="Last Updated" value={format(new Date(refund.update_on), 'MMM dd, yyyy HH:mm')} />
              )}
            </CardContent>
          </Card>
          ...
          {order && (
            <Card>
              ...
              <CardContent className="px-4 pb-4 divide-y divide-border">
                ...
                <Row label="Status"
                  value={
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_STYLES[order.status?.toLowerCase()] ?? STATUS_STYLES.pending
                      }`}>
                      {order.status}
                    </span>
                  }
                ...
                {order.voucher_code && (
                  <Row label="Voucher" value={<span className="font-mono text-xs">{order.voucher_code}</span>} />
                )}
                {order.delivery_notes && (
                  <Row label="Delivery Notes" value={order.delivery_notes} />
                )}
                {order.delivery_time && (
                  <Row label="Delivered At" value={format(new Date(order.delivery_time), 'MMM dd, yyyy HH:mm')} />
                )}
                <Row label="Order Created" value={format(new Date(order.created_at), 'MMM dd, yyyy HH:mm')} />
                {order.updated_at && (
                  <Row label="Order Updated" value={format(new Date(order.updated_at), 'MMM dd, yyyy HH:mm')} />
                )}
              </CardContent>
            </Card>
          )}
          ...
          {buyer && (
            <Card>
              ...
              <CardContent className="px-4 pb-4 divide-y divide-border">
                ...
                <Row label="Member Since" value={format(new Date(buyer.created_at), 'MMM dd, yyyy')} />
              </CardContent>
            </Card>
          )}
              ...
              <div className="flex gap-3 justify-end pb-4">
                {isPending && (
                  <Button
                    ...
                    disabled={isBusy}
                  >
                    {isUpdating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Search className="h-4 w-4 mr-2" />}
                    Start Review
                  </Button>
                )}
                {isInReview && (
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => approveRefund({ refund_id: refund.id, user_id: refund.user_id })}
                      disabled={isBusy}
                    >
