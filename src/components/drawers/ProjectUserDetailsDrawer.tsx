    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-xl overflow-y-auto border-l border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-0">
        <SheetHeader>
          ...
          <p className="sr-only">
            Comprehensive view of project user details including roles, contact information, and module-specific permissions.
          </p>
        </SheetHeader>
        ...
        <div className="mt-16 px-8 pb-8 space-y-8">
          <div>
            <h2 className="text-3xl font-black text-zinc-900 dark:text-zinc-100 tracking-tight">{user.username}</h2>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {accessibleModules.length > 0 ? (
                displayedModules.map((module) => (
                  <div key={module} className="flex items-center gap-2 p-2.5 rounded-xl border border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/40">
                    <div className="w-1.5 h-1.5 rounded-full bg-primary" />
                    <span className="text-[11px] font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-tight">{formatPrivilegeKey(module)}</span>
                  </div>
                ))
              ) : (
                <div className="col-span-2 text-center py-6 rounded-2xl border-2 border-dashed border-zinc-100 dark:border-zinc-800">
                  <p className="text-xs text-zinc-400 font-medium italic">No specific modal access configured</p>
                </div>
              )}
            </div>
            ...
            {accessibleModules.length > 12 && (
              ...
              >
                {showAllModules ? 'Show Less' : `View ${accessibleModules.length - 12} More Modules`}
              </Button>
            )}
          </div>

          <div className="space-y-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 ml-1">Security Configuration</h3>
            <div className="grid grid-cols-2 gap-4">
              <div className={`p-5 rounded-2xl border-2 ${user.TwoAuth_enabled ? 'bg-blue-50 border-blue-100 dark:bg-blue-900/10 dark:border-blue-900/20' : 'bg-zinc-50 border-zinc-100 dark:bg-zinc-800/30 dark:border-zinc-800'}`}>
                <Activity className={`w-6 h-6 mb-3 ${user.TwoAuth_enabled ? 'text-blue-500' : 'text-zinc-400'}`} />
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">Authenticator</p>
                <p className={`text-sm font-black ${user.TwoAuth_enabled ? 'text-blue-700 dark:text-blue-400' : 'text-zinc-500'}`}>
                  {user.TwoAuth_enabled ? 'SECURED' : 'UNSET'}
                </p>
              </div>
              <div className={`p-5 rounded-2xl border-2 ${user.sms_auth ? 'bg-orange-50 border-orange-100 dark:bg-orange-900/10 dark:border-orange-900/20' : 'bg-zinc-50 border-zinc-100 dark:bg-zinc-800/30 dark:border-zinc-800'}`}>
                <Smartphone className={`w-6 h-6 mb-3 ${user.sms_auth ? 'text-orange-500' : 'text-zinc-400'}`} />
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1">SMS Verification</p>
                <p className={`text-sm font-black ${user.sms_auth ? 'text-orange-700 dark:text-orange-400' : 'text-zinc-500'}`}>
          ...
          <div className="space-y-4 pt-4">
            <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 ml-1">System Audit Log</h3>
          ...
          {user.device_details && (
            <div className="space-y-4 pt-4">
              <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 ml-1">Verified Device Fingerprint</h3>
