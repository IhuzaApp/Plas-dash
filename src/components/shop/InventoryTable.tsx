...
const getStatusBadge = (status: string) => {
  switch (status) {
    case 'in-stock':
      return <Badge className="bg-emerald-500/10 text-emerald-600 hover:bg-emerald-500/20 border-emerald-200">In Stock</Badge>;
    case 'low-stock':
      return <Badge className="bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 border-amber-200">Low Stock</Badge>;
    case 'out-of-stock':
      return <Badge className="bg-rose-500/10 text-rose-600 hover:bg-rose-500/20 border-rose-200">Out of Stock</Badge>;
    default:
      return null;
  }
};
          ...
          <Table>
            <TableHeader className="bg-muted/50">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="text-right font-semibold text-foreground h-12">Price</TableHead>
                <TableHead className="text-right font-semibold text-foreground h-12">Stock</TableHead>
                <TableHead className="font-semibold text-foreground h-12">Status</TableHead>
                <TableHead className="text-right font-semibold text-foreground h-12 pr-6">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {paginatedItems.length > 0 ? (
                paginatedItems.map((item) => (
                  ...
                  >
                    <TableCell className="py-3">
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="flex flex-col">
                          <span className="font-semibold text-foreground whitespace-nowrap tracking-tight">{item.name}</span>
                          {item.measurement_unit && (
                            <span className="text-xs text-muted-foreground capitalize mt-0.5">{item.measurement_unit}</span>
                          )}
                        </div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell className="py-3">
                      {item.category ? (
                        <Badge variant="outline" className="font-medium bg-background/50 text-foreground/80 shadow-sm border-dashed">
                          {item.category}
                        </Badge>
                      ) : (
                    ...
                    <TableCell className="py-3">
                      <div className="flex items-center">
                        <span className="text-sm font-medium text-foreground/80">
                          {item.supplier || <span className="text-muted-foreground/50 italic">-</span>}
                        </span>
                      </div>
                    </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={8} className="h-48 text-center">
                    <div className="flex flex-col items-center justify-center text-muted-foreground">
                      ...
                      <p className="text-lg font-medium text-foreground">No inventory items found</p>
      ...
      {items.length > itemsPerPage && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2">
          <div className="text-sm text-muted-foreground font-medium">
            Showing <span className="text-foreground">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-foreground">{Math.min(currentPage * itemsPerPage, items.length)}</span> of <span className="text-foreground">{items.length}</span> results
          </div>
