    ...
    <div className="space-y-6">
        ...
        <Card className="border-none shadow-xl bg-gradient-to-br from-primary/10 via-background to-background rounded-3xl overflow-hidden">
          ...
          <CardContent className="space-y-4 pt-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm">
                    {subscriptionData?.end_date
                      ? format(new Date(subscriptionData.end_date), 'MMM dd, yyyy')
                      : 'N/A'}
                  </span>
                </div>
              </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-sm">
                    ...
                    {subscriptionData?.billing_cycle === 'yearly'
                      ? subscriptionData?.plan?.price_yearly
                      : subscriptionData?.plan?.price_monthly || '0.00'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 pt-2">
              <Button
                size="sm"
                className="w-full rounded-xl h-10 shadow-lg shadow-primary/20 bg-primary hover:scale-[1.02] transition-transform flex items-center justify-center gap-2"
              >
          ...
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <CardDescription>
                  Primary funding source for your business operations
                </CardDescription>
              </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <p className="text-4xl font-mono tracking-tighter font-bold text-primary">
                        {currencySymbol}
                        {(
                          subscriptionData?.Shop?.merchant_wallet?.balance ||
                          subscriptionData?.Restaurant?.merchant_wallet?.balance ||
                          0
                  ...
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm font-semibold uppercase tracking-wider">
                        {subscriptionData?.Shop?.name ||
                          subscriptionData?.Restaurant?.name ||
                          'Admin User'}
                      </p>
                    </div>
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <div className="p-4 rounded-2xl bg-background border border-primary/10 space-y-1">
                  ...
                  <p className="text-sm font-semibold">
                    {subscriptionData?.Shop?.merchant_wallet?.update_at
                      ? format(
                          new Date(subscriptionData.Shop.merchant_wallet.update_at),
                          'MMM dd, HH:mm'
                        )
                      : 'Recently'}
                  </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {(() => {
          ...
          const aiLimit =
            aiData?.request_count !== undefined
              ? aiData.request_count
              : subscriptionData?.plan?.ai_request_limit || 0;
          const aiUsed = aiData?.requests_sent || 0;
          const isUnlimited = aiLimit === -1;
          const aiPercentage = isUnlimited
            ? 0
            : aiLimit > 0
              ? Math.min(Math.round((aiUsed / aiLimit) * 100), 100)
              : 0;
                  ...
                  <div className="space-y-1">
                    ...
                    <CardDescription>
                      {hasNoSubscription
                        ...
                        : 'Consumption of AI credits for content generation'}
                    </CardDescription>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center py-4 space-y-4 text-center">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground max-w-[200px]">
                        Upgrade your plan to start using AI for your business content.
                      </p>
                    </div>
                  ...
                  <>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        ...
                        {!isUnlimited && (
                          <span className="text-primary font-bold">{aiPercentage}%</span>
                        )}
                      </div>
                      <Progress
                        value={isUnlimited ? 0 : aiPercentage}
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div className="space-y-1">
                        ...
                        <p className="text-sm font-bold">
                          {isUnlimited ? '∞' : aiLimit.toLocaleString()}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase font-bold text-muted-foreground">
                        ...
                        <p className="text-sm font-bold text-primary">
                          {isUnlimited ? '∞' : Math.max(0, aiLimit - aiUsed).toLocaleString()}
                        </p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[10px] uppercase font-bold text-muted-foreground">
                        ...
                        <p className="text-sm font-bold">
                          {subscriptionData?.end_date
                            ? Math.max(
                                0,
                                Math.ceil(
                                  (new Date(subscriptionData.end_date).getTime() -
                                    new Date().getTime()) /
                                    (1000 * 60 * 60 * 24)
                                )
                              )
        ...
        {(() => {
          ...
          const reelPercentage =
            reelLimit > 0 ? Math.min(Math.round((reelUsed / reelLimit) * 100), 100) : 0;
                  ...
                  <div className="space-y-1">
                    ...
                    <CardDescription>
                      {hasNoReelSubscription
                        ? 'Create stunning video marketing reels'
                        : 'Monthly video marketing generation quota'}
                    </CardDescription>
                  </div>
                ...
                {hasNoReelSubscription ? (
                  <div className="flex flex-col items-center justify-center py-4 space-y-4 text-center">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground max-w-[200px]">
                        Activate your reel production quota to start generating video content.
                      </p>
                    </div>
                    <Button className="w-full rounded-2xl bg-primary text-primary-foreground hover:bg-primary/90 transition-colors gap-2 shadow-lg shadow-primary/20">
                  ...
                  <>
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="font-semibold">
                          {reelUsed.toLocaleString()} / {reelLimit.toLocaleString()} Reels
                        </span>
                        <span className="text-primary font-bold">{reelPercentage}%</span>
                      </div>
                      <Progress
                        value={reelPercentage}
                        className="h-3 rounded-full bg-muted shadow-inner"
                      />
                    </div>
      ...
      <Card className="border-none shadow-xl rounded-3xl overflow-hidden bg-muted/10">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <Button
            variant="outline"
            className="rounded-xl border-primary/10 hover:bg-primary/5 text-primary gap-2"
          >
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-muted-foreground/10">
                  <th className="py-4 font-bold text-muted-foreground uppercase text-[10px] tracking-wider">
                    Invoice
                  </th>
                  <th className="py-4 font-bold text-muted-foreground uppercase text-[10px] tracking-wider">
                    Date
              ...
              <tbody>
                {subscriptionData?.subscription_invoices?.length > 0 ? (
                  subscriptionData.subscription_invoices.map((invoice: any) => (
                    <tr
                      key={invoice.id}
                      className="border-b border-muted-foreground/5 hover:bg-muted/5 transition-colors"
                    >
                      <td className="py-4">
                      ...
                      <td className="py-4">
                        <span className="font-bold">
                          {currencySymbol}
                          {invoice.plan_price?.toLocaleString()}
                        </span>
                      </td>
                      <td className="py-4">
                        <Badge
