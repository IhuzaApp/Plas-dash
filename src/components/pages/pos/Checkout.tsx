...
import { ShoppingBag, Plus, Minus, Trash, CreditCard, Banknote, CheckCircle, Clock, AlertCircle } from "lucide-react";
...
const Checkout = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      ...
      case "processing":
        return <Badge className="bg-blue-500"><AlertCircle className="mr-1 h-3 w-3" /> Processing</Badge>;
      default:
        return null;
    }
  };
                  ...
                  {pendingCheckouts.map((checkout) => (
                    <div key={checkout.id} className="p-4 border rounded-lg">
                      ...
                      <div className="flex space-x-2 mt-2">
                        ...
                        <Button size="sm" variant="outline" onClick={() => completePendingCheckout(checkout.id)}>
      ...
      <Dialog open={isTINDialogOpen} onOpenChange={setIsTINDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter className="mt-4">
            <Button variant="outline" onClick={() => setIsTINDialogOpen(false)}>Cancel</Button>
            <Button onClick={() => {
              setIsTINDialogOpen(false);
              processFinalCheckout();
            }} disabled={!tinNumber.trim()}>Continue</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
          ...
          {selectedPendingCheckout && (
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {pendingCheckouts.find(c => c.id === selectedPendingCheckout)?.timestamp.toLocaleString()}
                </p>
              </div>
              
              <ScrollArea className="h-[200px]">
                {pendingCheckouts.find(c => c.id === selectedPendingCheckout)?.items.map((item) => (
              ...
              <div className="pt-2">
                <div className="flex justify-between">
                  <span>Subtotal</span>
                  <span>${pendingCheckouts.find(c => c.id === selectedPendingCheckout)?.total.toFixed(2) || "0.00"}</span>
                </div>
                <div className="flex justify-between">
                  <span>Tax (8%)</span>
                  <span>
                    ${((pendingCheckouts.find(c => c.id === selectedPendingCheckout)?.total || 0) * 0.08).toFixed(2)}
                  </span>
                </div>
                ...
                <div className="flex justify-between font-bold">
                  <span>Total</span>
                  <span>
                    ${((pendingCheckouts.find(c => c.id === selectedPendingCheckout)?.total || 0) * 1.08).toFixed(2)}
                  </span>
                </div>
              </div>
