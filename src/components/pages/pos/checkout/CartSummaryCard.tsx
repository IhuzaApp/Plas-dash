...
// GraphQL query to get employee details
    ...
    if (selectedPaymentMethod) {
      ...
      try {
        ...
        if (autoGeneratedNumber) {
          ...
          let employee = null;
          try {
            const employeeResult = await hasuraRequest(GET_EMPLOYEE_BY_ID, { 
              id: currentUser?.id || '' 
            }) as any;
            employee = employeeResult.orgEmployees_by_pk;
    ...
    const printContent = `
      ...
      <body>
        <div class="receipt">
          ...
          <div class="items">
            ${lastPaymentDetails?.items?.map(item => `
              <div class="item">
