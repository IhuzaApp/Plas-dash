...
export default function POSBoard() {
  ...
  // Fetch list of branches (shops or restaurants)
  useEffect(() => {
    ...
    fetchBranches();
  }, [isAllowed, shopSession?.isRestaurant]);
  ...
  // Check if active user has administrative rights or has a multi-branch network to switch and view all branches
  const canSwitchBranches = useMemo(() => {
    if (relatedBranches.length > 1) return true;
  ...
  // Total combined store revenue generated
  const totalStoreRevenue = useMemo(() => {
    const isRestaurant = !!shopSession?.isRestaurant;
    if (isRestaurant) {
      const orders = shopData?.restaurant_orders || [];
      return orders.reduce((acc: number, ord: any) => acc + (parseFloat(ord.total) || 0), 0);
    } else {
      const checkouts = shopData?.shopCheckouts || [];
      return checkouts.reduce((acc: number, chk: any) => acc + (parseFloat(chk.total) || 0), 0);
    }
  }, [shopData, shopSession?.isRestaurant]);
  ...
  const combinedAnalytics = useMemo(() => {
    const data: any[] = [];
    const isRestaurant = !!shopSession?.isRestaurant;
    const txns: { date: Date; total: number }[] = [];

    if (isRestaurant) {
      const orders = shopData?.restaurant_orders || [];
      orders.forEach((ord: any) => {
        const dateStr = ord.created_at || ord.updated_at;
        if (dateStr) {
          txns.push({
      ...
      const checkouts = shopData?.shopCheckouts || [];
      checkouts.forEach((chk: any) => {
        const dateStr = chk.created_on;
        if (dateStr) {
          txns.push({
            date: new Date(dateStr),
    ...
    if (salesTimeframe === 'day') {
      ...
      txns.forEach(t => {
        ...
        const found = monthsList.find(m => m.year === tYear && m.month === tMonth);
        if (found) {
          found.Revenue += t.total;
          found.Clients += 1;
        }
      });
      data.push(...monthsList.map(m => ({ name: m.name, Revenue: m.Revenue, Clients: m.Clients })));
    }
  ...
  const filteredQueue = useMemo(() => {
    ...
    return pendingItems.filter((item: any) => {
      ...
      return id.includes(search) || title.includes(search) || desc.includes(search);
    });
