...
import {
    Search, Plus, Eye, Edit, Trash2, PlusCircle, X, ChefHat,
        ...
        <div className="rounded-xl border bg-muted/20 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {[
                    ...
                    <div key={row.label} className="bg-background rounded-lg border p-2.5">
                        ...
                        <p className={`text-sm mt-0.5 ${row.bold ? 'font-bold' : 'font-medium'}`}>{row.value}</p>
                    </div>
                ))}
            </div>
            <div className="flex flex-wrap gap-6 pt-1 border-t">
                ...
                <div>
                    ...
                    <p className={`text-base font-bold ${profitPerUnit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {fmt(profitPerUnit)}
                    </p>
                </div>
                <div>
...
export default function RecipeManagement() {
    ...
    const stats = useMemo(() => {
        ...
        const avgMargin = recipes.length
            ? recipes.reduce((s, r) => {
                ...
                return s + (r.sellingPrice > 0 ? ((r.sellingPrice - costPerUnit) / r.sellingPrice) * 100 : 0);
            }, 0) / recipes.length
            : 0;
    ...
    const updateIngField = (idx: number, field: 'quantity' | 'unit' | 'unitCost', val: number | string) => {
        setForm(p => {
            const ings = [...p.ingredients];
    ...
    const addIngredient = () => setForm(p => ({ ...p, ingredients: [...p.ingredients, blankIngredient()] }));
    ...
    const handleSave = () => {
        ...
        if (form.ingredients.some(i => !i.catalogueItemId)) { toast.error('Select an item for every ingredient row'); return; }

        const now = new Date().toISOString();
        if (dialogMode === 'create') {
            const newRecipe: Recipe = { id: `rec-${Date.now()}`, ...form, createdAt: now, updatedAt: now };
        ...
        <>
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                        { label: 'Total Recipes', value: stats.total, icon: ChefHat, color: 'text-violet-600', bg: 'bg-violet-100 dark:bg-violet-900/30' },
                        { label: 'Active', value: stats.active, icon: Layers, color: 'text-green-600', bg: 'bg-green-100 dark:bg-green-900/30' },
                        { label: 'Avg Margin', value: pct(stats.avgMargin), icon: TrendingUp, color: 'text-blue-600', bg: 'bg-blue-100 dark:bg-blue-900/30' },
                        { label: 'Categories', value: stats.categories, icon: DollarSign, color: 'text-amber-600', bg: 'bg-amber-100 dark:bg-amber-900/30' },
                    ].map(s => (
                                ...
                                <TableBody>
                                    {filtered.length > 0 ? filtered.map(recipe => {
                                        ...
                                        const margin = recipe.sellingPrice > 0
                                            ? ((recipe.sellingPrice - costPerUnit) / recipe.sellingPrice) * 100 : 0;
                                        return (
                                            <TableRow key={recipe.id} className="group">
                                                <TableCell className="font-medium">{recipe.name}</TableCell>
                                                <TableCell className="font-mono text-xs text-muted-foreground">{recipe.sku}</TableCell>
                                                <TableCell>
                                                    <Badge variant="outline" className={`text-xs ${CATEGORY_BADGE_STYLE[recipe.category] ?? ''}`}>
                                                        {recipe.category}
                                                    </Badge>
                                                </TableCell>
                                                <TableCell className="text-right text-sm font-medium">{fmt(costPerUnit)}</TableCell>
                                                <TableCell className="text-right text-sm">{fmt(recipe.sellingPrice)}</TableCell>
                                                ...
                                                <TableCell>
                                                    <Badge variant={recipe.isActive ? 'default' : 'secondary'} className="text-xs">
                                                        {recipe.isActive ? 'Active' : 'Inactive'}
                                                    </Badge>
                                                </TableCell>
                                                <TableCell className="text-right">
                                                    <div className="flex justify-end gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                                                        <Button variant="ghost" size="icon" title="View" onClick={() => openDialog('view', recipe)}>
                                                            <Eye className="h-4 w-4" />
                                                        </Button>
                                                        <Button variant="ghost" size="icon" title="Edit" onClick={() => openDialog('edit', recipe)}>
                ...
                <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            ...
                            {dialogMode === 'create' ? 'Create New Recipe' :
                                dialogMode === 'edit' ? 'Edit Recipe' : 'Recipe Details'}
                        </DialogTitle>
                        <DialogDescription>
                            {dialogMode === 'create' ? 'Define a new production recipe. All cost fields update in real time.' :
                                dialogMode === 'edit' ? 'Update recipe fields. Cost preview updates as you type.' :
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input value={form.name} disabled={isReadOnly}
                                        onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                                        placeholder="e.g. Classic Sourdough Loaf" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label>SKU <span className="text-destructive">*</span></Label>
                                    <Input value={form.sku} disabled={isReadOnly}
                                        onChange={e => setForm(p => ({ ...p, sku: e.target.value }))}
                                        placeholder="e.g. BRD-001" className="font-mono" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label>Category</Label>
                                    <Select value={form.category} disabled={isReadOnly}
                                ...
                                <div className="space-y-1.5">
                                    <Label>Status</Label>
                                    <Select value={form.isActive ? 'active' : 'inactive'} disabled={isReadOnly}
                                ...
                                <div className="sm:col-span-2 space-y-1.5">
                                    ...
                                    <Textarea value={form.description} disabled={isReadOnly} rows={2}
                        ...
                        <section className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                                <div className="space-y-1.5">
                                    ...
                                    <Input type="number" min={1} value={form.yieldQty} disabled={isReadOnly}
                                        onChange={e => setForm(p => ({ ...p, yieldQty: Number(e.target.value) }))} />
                                </div>
                                <div className="space-y-1.5">
                                    <Label>Yield Unit</Label>
                                    <Input value={form.yieldUnit} disabled={isReadOnly}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        <Input type="number" min={0} max={100} value={form.wastePct} disabled={isReadOnly}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Input type="number" min={0} step={0.01} value={form.sellingPrice} disabled={isReadOnly}
                                            className="pl-6"
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        <Input type="number" min={0} max={100} value={form.labourCostPct} disabled={isReadOnly}
                                            onChange={e => setForm(p => ({ ...p, labourCostPct: Number(e.target.value) }))} />
                                        <span className="absolute right-3 top-2.5 text-sm text-muted-foreground">%</span>
                                    </div>
                                </div>
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        <Input type="number" min={0} max={100} value={form.overheadCostPct} disabled={isReadOnly}
                                            onChange={e => setForm(p => ({ ...p, overheadCostPct: Number(e.target.value) }))} />
                                        <span className="absolute right-3 top-2.5 text-sm text-muted-foreground">%</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <section className="space-y-3">
                            ...
                            <div className="grid grid-cols-12 gap-2 px-2">
                                {['Ingredient', 'Qty', 'Unit', 'Unit Cost ($)', 'Subtotal', ''].map((h, i) => (
                                    <p key={i} className={`text-xs font-medium text-muted-foreground ${i === 0 ? 'col-span-4' : i === 1 ? 'col-span-2' : i === 2 ? 'col-span-2' : i === 3 ? 'col-span-2' : 'col-span-1'
                                        } ${i === 3 || i === 4 ? 'text-right' : ''}`}>{h}</p>
                                ))}
                            </div>
                                        ...
                                        <div key={ing.id}
                                            ...
                                            <div className="col-span-4">
                                                {isReadOnly ? (
                                                    ...
                                                    <Select value={ing.catalogueItemId}
                                                        ...
                                                        <SelectContent>
                                                            {INGREDIENT_CATALOGUE.map(cat => (
                                                                <SelectItem key={cat.id} value={cat.id}>
                                                                    {cat.name} <span className="text-muted-foreground ml-1">({cat.unit}, ${cat.unitCost})</span>
                                                                </SelectItem>
                                                            ))}
                                                        </SelectContent>
                                                    </Select>
                                            ...
                                            <div className="col-span-2">
                                                <Input className="h-8 text-sm" type="number" min={0} step={0.01}
                                                    value={ing.quantity} disabled={isReadOnly}
                                            ...
                                            <div className="col-span-2 text-right">
                                                <Input className="h-8 text-sm text-right" type="number" min={0} step={0.01}
                                                    value={ing.unitCost} disabled={isReadOnly}
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will permanently remove the recipe. Any associated production orders will be unaffected.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
