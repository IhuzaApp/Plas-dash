        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-muted-foreground mt-1">
                    Simulate ingredient stock deductions before committing to a production run. No actual stock is changed.
                </p>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 rounded-lg border border-blue-200 bg-blue-50 dark:bg-blue-950/30 dark:border-blue-900 text-blue-800 dark:text-blue-200">
                ...
                <p className="text-sm">
                    <strong>Frontend-only simulation.</strong> No inventory changes are made. Once backend is connected, stock figures will reflect live inventory data.
                </p>
            </div>
                ...
                <CardContent className="pt-5 pb-4">
                    <div className="flex flex-col sm:flex-row gap-4 items-end">
                        <div className="space-y-1.5 flex-1">
                            ...
                            <Select value={recipeId} onValueChange={id => { setRecipeId(id); setHasSimulated(false); }}>
                        ...
                        <div className="space-y-1.5 w-40">
                            ...
                            <Input type="number" min={1} max={50} value={batches}
                                onChange={e => { setBatches(Math.max(1, Number(e.target.value))); setHasSimulated(false); }} />
                        </div>
            ...
            <p className="text-sm text-muted-foreground">
                ...
                &nbsp;·&nbsp;Yield: {recipe.yieldQty * batches} {recipe.yieldUnit} from {batches} batch{batches !== 1 ? 'es' : ''}
                ...
                <>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Stock levels <em>after</em> producing {batches} batch{batches !== 1 ? 'es' : ''} of <strong>{recipe.name}</strong>
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="px-0">
                            <Table>
                                ...
                                <TableBody>
                                    {result.map((item, i) => (
                                        <TableRow key={i} className={!item.isSufficient ? 'bg-red-50/50 dark:bg-red-950/10' : item.willBeLow ? 'bg-amber-50/50 dark:bg-amber-950/10' : ''}>
                                            <TableCell className="font-medium text-sm">{item.name}</TableCell>
                                            <TableCell className="text-right text-sm">{item.currentStock} {item.unit}</TableCell>
                                            <TableCell className="text-right text-sm font-semibold">{item.required.toFixed(2)} {item.unit}</TableCell>
                                            <TableCell className={`text-right text-sm font-bold ${!item.isSufficient ? 'text-red-600' : item.willBeLow ? 'text-amber-600' : 'text-green-600'}`}>
                                                {item.isSufficient ? `${item.remaining.toFixed(2)} ${item.unit}` : `−${Math.abs(item.remaining).toFixed(2)} ${item.unit}`}
                                            </TableCell>
                                            <TableCell className="text-right text-sm text-muted-foreground">{item.lowStockThreshold} {item.unit}</TableCell>
                                            <TableCell>
                                                {!item.isSufficient ? (
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        {[
                            { label: 'OK', count: healthy.length, color: 'text-green-700', bg: 'bg-green-50 dark:bg-green-950/20 border-green-200' },
                            { label: 'Will be Low', count: willBeLow.length, color: 'text-amber-700', bg: 'bg-amber-50 dark:bg-amber-950/20 border-amber-200' },
                            { label: 'Insufficient', count: insufficient.length, color: 'text-red-700', bg: 'bg-red-50 dark:bg-red-950/20 border-red-200' },
                        ].map(s => (
            ...
            {!hasSimulated && (
                <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground gap-3">
                    ...
                    <p className="text-sm">Select a recipe and batch count, then click <strong>Run Simulation</strong>.</p>
                </div>
            )}
        </div>
    );
