...
export default function ProductionSection() {
  ...
  const stockCheck = useMemo(
    () => checkStock(selectedRecipe, batchQty, stock),
    [selectedRecipe, batchQty, stock]
  );
  const hasInsufficientStock = stockCheck.some(l => !l.isSufficient);
  const recipeCost = useMemo(
    () => computeFullCost(selectedRecipe, batchQty),
    [selectedRecipe, batchQty]
  );
  ...
  const handleSimulate = useCallback(() => {
    if (hasInsufficientStock) {
      toast.error('Insufficient stock to simulate production');
      return;
    }
    setSimulating(true);
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <SummaryCard
            ...
            sub={
              todayStats.estRevenue > 0
                ? `${((todayStats.estProfit / todayStats.estRevenue) * 100).toFixed(1)}% margin`
                : undefined
            }
            icon={TrendingUp}
          ...
          <div className="flex items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-900 p-4">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-amber-800 dark:text-amber-300">
                Low Raw Material Stock Alert ({lowStockItems.length} item
              ...
              <div className="flex flex-wrap gap-3">
                {Object.entries(finishedGoods).map(([id, qty]) => {
                  ...
                  return (
                    <div
                      key={id}
                      className="flex items-center gap-2 px-3 py-2 rounded-lg bg-green-100 dark:bg-green-900/30 border border-green-200 dark:border-green-800"
                    >
                ...
                <TableBody>
                  {orders.length > 0 ? (
                    orders.map(order => {
                      ...
                      const costTotal = recipe
                        ? computeFullCost(recipe, order.batchQty).totalCost
                        : order.totalIngredientCost;
                      ...
                      return (
                        <TableRow key={order.id} className="group">
                          ...
                          <TableCell>
                            <Badge
                              variant="outline"
                              className={`text-xs flex items-center gap-1 w-fit ${cfg.badge}`}
                            >
                              {cfg.icon}
                              {cfg.label}
                            </Badge>
      ...
      <Dialog open={createOpen} onOpenChange={open => !open && handleCloseCreate()}>
        <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Select a recipe and quantity. Ingredient availability and cost are calculated in real
              time. Click <strong>Simulate Production</strong> to commit the run and update mock
              stock.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 py-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>
                  Select Recipe <span className="text-destructive">*</span>
                </Label>
            ...
            <div className="rounded-lg bg-muted/30 border p-3 text-sm flex flex-wrap gap-4">
              ...
              <span>
                <strong>Yield:</strong> {selectedRecipe.yieldQty * batchQty}{' '}
                {selectedRecipe.yieldUnit}
              </span>
              <span>
                <strong>Waste:</strong> {selectedRecipe.wastePct}%
              </span>
            </div>
            ...
            <div className="space-y-2">
              ...
              {hasInsufficientStock && (
                <div className="flex items-center gap-2 p-3 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 text-red-700 dark:text-red-300 text-sm">
                  ...
                  <strong>Insufficient stock</strong> — some ingredients cannot meet requirements
                  for {batchQty} batch{batchQty !== 1 ? 'es' : ''}.
                </div>
              )}
                  ...
                  <TableBody>
                    {stockCheck.map((line, i) => (
                      ...
                      >
                        ...
                        <TableCell
                          className={`text-right text-sm py-2 font-bold ${
                            !line.isSufficient
                              ? 'text-red-600'
                              : line.willBeLow
                                ? 'text-amber-600'
                        ...
                        <TableCell className="py-2">
                          {!line.isSufficient ? (
                            <Badge
                              variant="outline"
                              className="bg-red-100 text-red-700 border-red-200 text-xs gap-1"
                            >
            ...
            <div className="rounded-xl border bg-muted/20 p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: 'Ingredient Cost', value: fmt(recipeCost.ingredientCost), color: '' },
                  {
                    ...
                    value: fmt(recipeCost.totalCost),
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <strong>
                  {' '}
                  {selectedRecipe.yieldQty * batchQty} {selectedRecipe.yieldUnit}
                </strong>
      ...
      <Dialog open={!!viewOrder} onOpenChange={() => setViewOrder(null)}>
        {viewOrder &&
          (() => {
            ...
            const cost = recipe
              ? computeFullCost(recipe, viewOrder.batchQty).totalCost
              : viewOrder.totalIngredientCost;
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-3 py-2 text-sm">
                  {[
                    ...
                    [
                      'Total Cost',
                      <span key="c" className="font-bold text-rose-600">
                        {fmt(cost)}
                      </span>,
                    ],
                    [
                      'Est. Revenue',
                      <span key="r" className="font-bold text-green-600">
                        {fmt(revenue)}
                      </span>,
                    ],
                    [
                      ...
                      <span
                        key="p"
                        className={`font-bold ${revenue - cost >= 0 ? 'text-emerald-600' : 'text-red-600'}`}
                      >
                        {fmt(revenue - cost)}
                      </span>,
                    ],
