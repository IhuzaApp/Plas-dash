...
const STATUS_CONFIG: Record<string, { color: string; icon: React.ReactNode }> = {
    Completed: { color: 'bg-green-500/10 text-green-700 border-green-200', icon: <CheckCircle2 className="h-4 w-4 text-green-600" /> },
    'In Progress': { color: 'bg-amber-500/10 text-amber-700 border-amber-200', icon: <Clock className="h-4 w-4 text-amber-600" /> },
    Draft: { color: 'bg-slate-500/10 text-slate-700 border-slate-200', icon: <Activity className="h-4 w-4 text-slate-500" /> },
    Cancelled: { color: 'bg-red-500/10 text-red-700 border-red-200', icon: <AlertCircle className="h-4 w-4 text-red-500" /> },
};
...
export default function ProductionDashboard() {
    ...
    const totalUnitsProduced = DUMMY_PRODUCTION_ORDERS.filter(o => o.status === 'Completed').reduce((sum, o) => {
                    ...
                    <CardContent>
                        <div className="h-[280px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={PRODUCTION_MONTHLY_TREND} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                                    ...
                                    <Legend />
                                    <Line yAxisId="left" type="monotone" dataKey="batches" stroke="#6366f1" strokeWidth={2.5} dot={{ r: 4 }} name="Batches" />
                                    <Line yAxisId="right" type="monotone" dataKey="units" stroke="#22c55e" strokeWidth={2.5} dot={{ r: 4 }} name="Units" />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </CardContent>
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent>
                        <div className="h-[240px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={RECIPE_PRODUCTION_VOLUME} layout="vertical" margin={{ top: 5, right: 24, left: 4, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="hsl(var(--border))" />
                                    <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 12 }} />
                                    <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fill: 'hsl(var(--muted-foreground))', fontSize: 11 }} width={140} />
                                    <Tooltip contentStyle={{ borderRadius: '8px', border: '1px solid hsl(var(--border))' }} formatter={(v: number) => [v, 'Completed Batches']} />
                                ...
                                return (
                                    <div key={order.id} className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-muted-foreground">{order.batchQty} batch{order.batchQty !== 1 ? 'es' : ''} · {order.assignedStaff}</p>
                                        </div>
