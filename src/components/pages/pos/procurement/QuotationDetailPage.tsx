...
import { ArrowLeft, Building2, Calendar, Mail, Phone, MapPin, CheckCircle, FileText, ShoppingCart, TrendingDown } from 'lucide-react';
...
export default function QuotationDetailPage({ quotationId }: QuotationDetailPageProps) {
    ...
    const supplier = useMemo(() => DUMMY_SUPPLIERS.find(s => s.id === quotation?.supplierId), [quotation]);
        ...
        <div className="space-y-6 pb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight">{quotation.rfqNumber}</h2>
                            <Badge variant={
                                quotation.status === 'Accepted' ? 'default' :
                                    quotation.status === 'Received' ? 'secondary' :
                                        quotation.status === 'Sent' ? 'outline' :
                        ...
                        <p className="text-muted-foreground mt-1 flex items-center gap-2">
                            ...
                            Requested on {format(new Date(quotation.dateRequested), 'MMMM dd, yyyy')} •
                            Valid until {format(new Date(quotation.dateValidUntil), 'MMMM dd, yyyy')}
                        </p>
                    </div>
                </div>
                                        ...
                                        {enhancedItems.map((item, index) => (
                                            <TableRow key={index}>
                                                ...
                                                <TableCell className="text-right">
                                                    {item.quantity} <span className="text-muted-foreground text-xs">{item.stockUnit}</span>
                                                </TableCell>
                                                <TableCell className="text-right">
                                ...
                                <div className="w-full sm:w-1/2 md:w-1/3 space-y-3">
                                    ...
                                    {quotation.status === 'Received' && (
                                        <div className="mt-4 p-3 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-md text-sm flex items-start gap-2">
                                            ...
                                            <p>This quote is approximately <strong>8% below</strong> the median market rate for these items.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="h-[300px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <Bar dataKey={supplier.name} fill="#3b82f6" radius={[4, 4, 0, 0]} maxBarSize={50} />
