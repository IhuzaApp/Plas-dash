    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" onClick={() => router.push('/pos/procurement/suppliers')} className="rounded-full bg-slate-100 hover:bg-slate-200">
                ...
                <div>
                    ...
                    <p className="text-muted-foreground flex items-center gap-2 mt-1">
                        ...
                        {supplier.taxId && <> <span className="text-slate-300">|</span> Tax ID: {supplier.taxId} </>}
                    </p>
                </div>
            </div>
                ...
                <div className="space-y-6 lg:col-span-1">
                    ...
                    <Card className="shadow-sm border-0 ring-1 ring-slate-200">
                        ...
                        <CardContent className="pt-6 space-y-4">
                            <div>
                                <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Company Name</h4>
                                <p className="font-medium text-slate-900">{supplier.companyName || supplier.name}</p>
                                <Badge variant="outline" className="mt-1 bg-slate-50">{supplier.category}</Badge>
                            </div>
                            ...
                            <div className="space-y-3 pt-2 border-t text-sm">
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <a href={`tel:${supplier.phone}`} className="text-slate-900 hover:text-primary transition-colors">{supplier.phone}</a>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-slate-900">{supplier.address || 'No address provided'}</div>
                                    </div>
                                </div>
                            </div>
                        </CardContent>
                    ...
                    <Card className="shadow-sm border-0 ring-1 ring-slate-200">
                        ...
                        <CardContent className="pt-6 space-y-5">
                            <div className="flex items-center justify-between">
                                ...
                                <Badge variant="secondary" className="font-medium bg-slate-100">{supplier.paymentTerms}</Badge>
                            </div>

                            <div className="flex items-center justify-between">
                                <span className="text-sm font-medium text-slate-500">Credit Limit</span>
                                <span className="font-semibold text-slate-900">{formatMoney(supplier.creditLimit || 0)}</span>
                            </div>
                                ...
                                {supplier.creditLimit && supplier.creditLimit > 0 && (
                                    <div className="mt-3">
                                        <div className="flex justify-between text-xs mb-1 text-slate-500">
                                            ...
                                            <span>{Math.round((supplier.outstandingBalance / supplier.creditLimit) * 100)}%</span>
                                        </div>
                                        <div className="w-full bg-slate-200 rounded-full h-1.5 overflow-hidden">
                                            <div
                                                className={`h-1.5 rounded-full ${supplier.outstandingBalance > supplier.creditLimit * 0.8 ? 'bg-rose-500' : 'bg-primary'}`}
                                                style={{ width: `${Math.min((supplier.outstandingBalance / (supplier.creditLimit)) * 100, 100)}%` }}
                                            />
                                        </div>
                                    </div>
                                )}
                            ...
                            <CardContent className="p-4 sm:p-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-2xl font-bold text-slate-900">{formatMoney(totalOrdersValue)}</div>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                            ...
                            <CardContent className="p-4 sm:p-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-2xl font-bold text-slate-900">{supplier.leadTimeDays} <span className="text-sm font-medium text-slate-500">days</span></div>
                                    </div>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="shadow-sm border-0 ring-1 ring-slate-200">
                            ...
                            <CardContent className="h-[250px] pt-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={purchasesData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <CardContent className="h-[250px] pt-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <LineChart data={balanceData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                        ...
                                        />
                                        <Line type="monotone" dataKey="balance" stroke="#EF4444" strokeWidth={3} dot={{ r: 4, fill: '#EF4444', strokeWidth: 0 }} activeDot={{ r: 6 }} />
                                    </LineChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                        ...
                        <CardContent className="p-0">
                            {supplierPos.length > 0 ? (
                                        ...
                                        <TableBody>
                                            {supplierPos.slice(0, 5).map(po => (
                                                <TableRow key={po.id} className="hover:bg-slate-50/80 transition-colors">
                                                    ...
                                                    <TableCell className="text-right font-medium">{formatMoney(po.totalAmount)}</TableCell>
                                                    <TableCell>
                                                        <Badge variant={
                                                            ...
                                                            po.status === 'Delivered' ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-100 border-0' :
                                                                po.status === 'Approved' ? 'bg-blue-100 text-blue-800 hover:bg-blue-100 border-0' :
                                                                    po.status === 'Shipped' ? 'bg-indigo-100 text-indigo-800 hover:bg-indigo-100 border-0' :
                                                                        ''
                                                        }>
                                                            {po.status}
                                                        </Badge>
                                                    ...
                                                    <TableCell className="text-right">
                                                        <Button variant="ghost" size="sm" className="h-8 text-primary hover:text-primary hover:bg-primary/10">View details</Button>
                                                    </TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                ...
                                <div className="p-8 text-center flex flex-col items-center">
                                    ...
                                    <p className="text-sm text-slate-400 mt-1">This supplier doesn&apos;t have any purchase history yet.</p>
                                </div>
                            )}
                            {supplierPos.length > 5 && (
                                <div className="p-3 border-t bg-slate-50/50 text-center">
                                    <Button variant="link" className="text-sm text-primary">View all {supplierPos.length} purchase orders</Button>
                                </div>
                            )}
                        </CardContent>
                    </Card>
