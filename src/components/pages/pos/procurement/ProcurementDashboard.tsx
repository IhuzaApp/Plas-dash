...
export default function ProcurementDashboard() {
    ...
    const outstandingPayables = DUMMY_PURCHASE_ORDERS
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                                ...
                                <h3 className="text-2xl font-bold mt-1 truncate text-primary">
                                    {currency}{totalPurchases.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </h3>
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                                ...
                                <h3 className="text-2xl font-bold mt-1 truncate text-rose-600">
                                    {currency}{outstandingPayables.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                </h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="h-[300px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <LineChart data={monthlyTrendData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                                    ...
                                    <YAxis axisLine={false} tickLine={false} tick={{ fill: '#6B7280', fontSize: 12 }} tickFormatter={(val) => `${currency}${val / 1000}k`} />
                                    <Tooltip
                                        contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB' }}
                                        formatter={(value: number) => [`${currency}${value.toLocaleString()}`, 'Purchases']}
                                    />
                                    <Line type="monotone" dataKey="purchases" stroke="#2563eb" strokeWidth={3} dot={{ r: 4, fill: '#2563eb' }} activeDot={{ r: 6 }} />
                                </LineChart>
                            </ResponsiveContainer>
                        </div>
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="h-[350px] w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={outstandingData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                    <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#E5E7EB" />
                                    <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: '#6B7280', fontSize: 12 }} tickFormatter={(val) => `${currency}${val / 1000}k`} />
                                    <YAxis type="category" dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#4B5563', fontSize: 12 }} width={120} />
                                    <Tooltip
                                        cursor={{ fill: 'transparent' }}
                                        contentStyle={{ borderRadius: '8px', border: '1px solid #E5E7EB' }}
                                        formatter={(value: number) => [`${currency}${value.toLocaleString()}`, 'Outstanding Balance']}
                                    />
