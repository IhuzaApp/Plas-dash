...
import { Building2, Search, Plus, Edit, Trash2, Mail, Phone, Eye, DollarSign, Users, CreditCard, FileText } from 'lucide-react';
...
export default function SuppliersPage() {
    ...
    const onCreditSuppliers = DUMMY_SUPPLIERS.filter(s => s.outstandingBalance > 0 || s.paymentTerms !== 'Cash').length;
                                    ...
                                    paginatedSuppliers.map((supplier) => (
                                        <TableRow key={supplier.id} className="group">
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary flex-shrink-0 group-hover:bg-primary group-hover:text-white transition-colors">
                                                        <span className="font-bold text-sm tracking-widest">{supplier.name.substring(0, 2).toUpperCase()}</span>
                                                    </div>
                                                    <div>
                                            ...
                                            <TableCell className="text-right">
                                                <div className={`font-medium ${supplier.outstandingBalance > 0 ? 'text-rose-600' : 'text-slate-600'}`}>
                                            ...
                                            <TableCell>
                                                ...
                                                } className={supplier.status === 'Active' ? 'bg-emerald-500/10 text-emerald-700 hover:bg-emerald-500/20 border-0' : supplier.status === 'Pending' ? 'bg-amber-500/10 text-amber-700 hover:bg-amber-500/20 border-0' : ''}>
                                                    {supplier.status}
                                                </Badge>
                                            </TableCell>
                                            <TableCell className="text-right">
                                                ...
                                                <div className="flex items-center justify-end gap-1 md:hidden">
                                                    <Button variant="ghost" size="icon" className="h-8 w-8 text-blue-600" onClick={() => handleViewClick(supplier)}>
                                                        <Eye className="w-4 h-4" />
                                                    </Button>
                                                </div>
                                            </TableCell>
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center justify-between px-4 py-3 border-t bg-slate-50/50">
                            <div className="text-sm text-slate-500">
                                Showing {((currentPage - 1) * ITEMS_PER_PAGE) + 1} to {Math.min(currentPage * ITEMS_PER_PAGE, filteredSuppliers.length)} of {filteredSuppliers.length} entries
                            </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="name" defaultValue={selectedSupplier?.name} placeholder="e.g. FreshFarm" />
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="companyName">Full Company Name</Label>
                                    <Input id="companyName" defaultValue={selectedSupplier?.companyName || selectedSupplier?.name} placeholder="e.g. FreshFarm Produce Co." />
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="category">Supplier Category <span className="text-destructive">*</span></Label>
                                    <Input id="category" defaultValue={selectedSupplier?.category} placeholder="e.g. Produce, Dairy" />
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="taxId">Tax ID / Registration No.</Label>
                                    <Input id="taxId" defaultValue={selectedSupplier?.taxId} placeholder="e.g. TX-12345" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Input id="contact" defaultValue={selectedSupplier?.contactPerson} placeholder="Full Name" />
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="email">Email Address <span className="text-destructive">*</span></Label>
                                    <Input id="email" type="email" defaultValue={selectedSupplier?.email} placeholder="email@company.com" />
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="phone">Phone Number <span className="text-destructive">*</span></Label>
                                    <Input id="phone" type="tel" defaultValue={selectedSupplier?.phone} placeholder="+1 555-0100" />
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="address">Physical Address</Label>
                                    <Input id="address" defaultValue={selectedSupplier?.address} placeholder="Street, City, Zip" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 md:col-span-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="paymentTerms">Payment Terms <span className="text-destructive">*</span></Label>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input id="creditLimit" type="number" defaultValue={selectedSupplier?.creditLimit || 0} />
                                    </div>

                                    <div className="space-y-2">
                                        <Label htmlFor="notes">Internal Notes</Label>
                                        <Input id="notes" defaultValue={selectedSupplier?.notes} placeholder="Additional info..." />
                                    </div>
                                </div>
                            </div>
                        </div>
