...
const log = {
  warn: (msg: string, context?: string, data?: any) =>
    console.warn(`[${context || 'PindoLib'}] ${msg}`, data || ''),
  error: (msg: string, context?: string, data?: any) =>
    console.error(`[${context || 'PindoLib'}] ${msg}`, data || ''),
  info: (msg: string, context?: string, data?: any) =>
    console.info(`[${context || 'PindoLib'}] ${msg}`, data || ''),
};
...
export const sendSMS = async (to: string, text: string) => {
  if (!pindoToken || !pindo) {
    log.warn('[MOCK] SMS sending disabled due to missing PINDO_API_TOKEN', 'PindoLib', {
      to,
      text,
    });
