...
export class InvoiceGenerator {
    ...
    public static async generate(invoice: InvoiceData, action: 'download' | 'view' = 'download') {
        ...
        doc.text(`${invoice.currency} ${subtotal.toLocaleString()}`, 190, footerStart, { align: 'right' });

        if (tax > 0) {
            doc.text('Tax:', 140, footerStart + 7);
            doc.text(`${invoice.currency} ${tax.toLocaleString()}`, 190, footerStart + 7, { align: 'right' });
        }

        if (discount > 0) {
            doc.setTextColor(200, 0, 0);
            doc.text('Discount:', 140, footerStart + 14);
            doc.text(`-${invoice.currency} ${discount.toLocaleString()}`, 190, footerStart + 14, { align: 'right' });
            doc.setTextColor(0);
        }
        ...
        doc.text(`${invoice.currency} ${total.toLocaleString()}`, 190, footerStart + 25, { align: 'right' });
