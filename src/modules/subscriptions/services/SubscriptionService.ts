...
export class SubscriptionService {
  ...
  async handleSubscriptionAssignment(payload: any, userId?: string) {
    ...
    if (payload.shop_id || payload.restaurant_id || payload.business_id) {
      ...
      const existingData = await hasuraClient.request<any>(
        gql`
    ...
    // 2. Get Plan details
    const planData = await hasuraClient.request<any>(
      gql`
    ...
    if (existingSub) {
      ...
      const insertResult = await hasuraClient.request<any>(INSERT_SHOP_SUBSCRIPTION, {
        object: subscriptionInsertObject,
      });
  ...
  async processAutomatedStatusUpdates() {
    ...
    try {
      const data = await hasuraClient.request<{ shop_subscriptions: any[] }>(
        GET_SUBSCRIPTIONS_FOR_AUTOMATION
      );
      ...
      for (const sub of subscriptions) {
        ...
        if (diffDays <= 14 && diffDays > -1) {
          try {
            ...
            const existingInvoices = await hasuraClient.request<any>(
              gql`
