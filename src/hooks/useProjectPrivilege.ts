...
import { ProjectUserPrivileges, ProjectPrivilegeKey, hasProjectPrivilege } from '@/types/projectPrivileges';
...
export function useProjectPrivilege(): UseProjectPrivilegeReturn {
  ...
  const hasProjectAction = useCallback(<T extends ProjectPrivilegeKey>(
    module: T,
    action: keyof NonNullable<ProjectUserPrivileges[T]>
  ): boolean => {
    const privileges = getProjectPrivileges();
