...
export function AssignSubscriptionDialog({ open, onOpenChange }: AssignSubscriptionDialogProps) {
    ...
    const { data: businessData, isLoading: businessLoading } = useQuery<{ business_accounts: BusinessAccount[] }>({
        queryKey: ['business-accounts'],
        queryFn: () => apiGet<{ business_accounts: BusinessAccount[] }>('/api/queries/business-accounts'),
    ...
    const { data: restaurantsData, isLoading: restaurantsLoading } = useQuery<{ restaurants: Restaurant[] }>({
