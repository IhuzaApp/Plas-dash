...
export default function ModulesPage() {
    ...
    const handleSyncModules = async () => {
        setIsSyncing(true);
        try {
            const res = await apiPost<{ message: string; count: number }>('/api/subscriptions/sync-modules', {});
            toast({
                                ...
                                return (
                                    <React.Fragment key={category}>
                                        ...
                                        >
                                            <TableCell colSpan={5} className="py-2">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Badge variant="secondary" className="ml-2 h-5 px-1.5 py-0 text-[10px] font-medium">
                                                        {modules.length}
                                                    </Badge>
                                                </div>
                                            </TableCell>
                                        ...
                                        {isExpanded && modules.map((mod) => (
                                            <TableRow key={mod.id} className="animate-in fade-in slide-in-from-top-1 duration-200">
