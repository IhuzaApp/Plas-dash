...
export default function BusinessProfilePage() {
    ...
    switch (status) {
      ...
      case 'active':
        return (
          <Badge
            variant="outline"
            className="text-green-600 border-green-600 bg-green-50 capitalize"
          >
            {status}
          </Badge>
        );
  ...
  const handleDelete = () => {
    ...
    if (
      confirm(
        `CRITICAL WARNING: Are you absolutely sure you want to permanently delete the business account "${data.business_name}"? This action cannot be undone.`
      )
    ) {
  ...
  const getOrderChartData = () => {
    ...
    return Object.keys(orderCounts)
      .map(date => ({
        name: date,
        ...
        <AdminLayout>
          <div className="p-6">
            ...
            <Alert variant="destructive">
              ...
              <AlertDescription>
                {(error as Error)?.message || 'Failed to load business profile.'}
              </AlertDescription>
            </Alert>
          </div>
        </AdminLayout>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {data.rfqs_count} / {data.quotes_count}
                </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        <h4 className="text-sm font-medium">
                          Provided Products/Services by Category
                        </h4>
                        <div className="h-[250px] w-full border rounded-md p-4 bg-muted/20">
                        ...
                        {stores?.length > 0 ? (
                          stores.map((store: any) => (
                            ...
                            >
                              ...
                              <TableCell>
                                {store.PlasBusinessProductsOrSerives?.length || 0}
                              </TableCell>
                        ...
                        {rfqs?.length > 0 ? (
                          rfqs.map((rfq: any) => (
                            <TableRow key={rfq.id}>
                              ...
                              <TableCell>
                                {rfq.min_budget
                                  ? `${currency}${rfq.min_budget} - ${currency}${rfq.max_budget}`
                                  : 'Not specified'}
                              </TableCell>
                            ...
                            <CardContent className="p-4 flex items-center justify-between">
                              <div className="flex items-center gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-2xl font-bold">
                                    {currency}
                                    {wallet.amount ? Number(wallet.amount).toFixed(2) : '0.00'}
                                  </p>
                                </div>
                              </div>
                              <div className="text-right">
                                ...
                                <p className="font-medium">
                                  {wallet.businessTransactions?.length || 0}
                                </p>
                              </div>
                            </CardContent>
                          </Card>
