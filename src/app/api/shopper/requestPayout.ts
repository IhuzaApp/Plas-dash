...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') {
    ...
    return res.status(405).json({ error: `Method ${req.method} Not Allowed`, message: '' });
  }
  ...
  try {
    ...
    if (!phoneNumber || typeof phoneNumber !== 'string') {
      ...
      return res.status(400).json({
        ...
        message: 'Please enter the phone number where you want to receive the funds.',
      });
    }
    ...
    if (!stored) {
      ...
      return res.status(400).json({
        ...
        message: 'OTP not found or expired. Click “Send OTP” again and enter the new code.',
      });
    }
    ...
    if (stored.otp !== String(otpCode)) {
      ...
      return res.status(400).json({
        ...
        message: 'The code you entered is wrong. Check the popup and try again.',
      });
    }
    ...
    const walletData = await hasuraClient.request<GraphQLWalletResponse>(GET_SHOPPER_WALLET, {
      shopper_id: userId,
    });
