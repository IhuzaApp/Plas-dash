...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const availableRestaurantOrders = restaurantOrders.map(order => {
      ...
      const pendingMinutes = Math.floor((Date.now() - referenceDate.getTime()) / 60000);
      ...
      const totalItems =
        order.restaurant_order_items?.reduce((sum, dish) => sum + (dish.quantity || 0), 0) || 1;
    ...
    logger.info('Filtered orders', 'AvailableOrders', {
      ...
      restaurantOrderCount: filteredOrders.filter(o => o.orderType === 'restaurant').length,
      maxTravelTime: `${maxTravelTime} minutes`,
    });
