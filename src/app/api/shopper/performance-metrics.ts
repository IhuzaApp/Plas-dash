...
const GET_PERFORMANCE_METRICS = gql`
  query GetPerformanceMetrics($shopperId: uuid!, $thirtyDaysAgo: timestamptz!) {
    ...
    AssignedOrders: Orders_aggregate(
      where: { shopper_id: { _eq: $shopperId }, created_at: { _gte: $thirtyDaysAgo } }
    ) {
      aggregate {
        count
      }
    ...
    RecentOrders: Orders(
      where: { shopper_id: { _eq: $shopperId }, created_at: { _gte: $sevenDaysAgo } }
    ) {
      id
      created_at
      updated_at
...
const calculateDeliveryTimePerformance = (orders: any[]): number => {
  const deliveredOrders = orders.filter(
    order => order.status === 'delivered' && order.delivery_photo_url && order.delivery_time // Regular orders have delivery_time
  );
  ...
  const onTimeDeliveries = deliveredOrders.filter(order => {
    try {
      ...
      const timeDifference = Math.abs(actualDelivery.getTime() - estimatedDelivery.getTime());
...
const calculateCombinedDeliveryTimePerformance = (
  ...
  const deliveredRegularOrders = regularOrders.filter(
    order => order.status === 'delivered' && order.delivery_photo_url && order.delivery_time
  );
  ...
  const totalDelivered = deliveredRegularOrders.length + deliveredReelOrders.length;
  ...
  const onTimeRegularDeliveries = deliveredRegularOrders.filter(order => {
    try {
      ...
      logger.error('Error calculating regular order delivery time', 'PerformanceMetrics', {
        error,
        orderId: order.id,
      });
      return false;
...
const calculateResponseTime = (orders: any[]): number => {
  const acceptedOrders = orders.filter(
    order => order.shopper_id && order.status !== 'PENDING' && order.created_at && order.updated_at
  );
  ...
  const responseTimes = acceptedOrders.map(order => {
    try {
      ...
      return Math.max(0, (acceptedTime.getTime() - assignedTime.getTime()) / 1000); // seconds
    } catch (error) {
...
const calculateCancellationRate = (orders: any[]): number => {
  ...
  const cancelledOrders = assignedOrders.filter(
    order => !['delivered', 'shopping', 'packing', 'on_the_way'].includes(order.status)
  );
...
const calculateOverallScore = (metrics: Partial<PerformanceMetrics>): number => {
  const weights = {
  ...
  const customerRatingScore = (metrics.customerRating || 0) * 20 * weights.customerRating;
  const onTimeDeliveryScore = (metrics.onTimeDelivery || 0) * weights.onTimeDelivery;
  const orderAccuracyScore = (metrics.orderAccuracy || 0) * weights.orderAccuracy;
  const acceptanceRateScore = (metrics.acceptanceRate || 0) * weights.acceptanceRate;

  const totalScore =
    customerRatingScore + onTimeDeliveryScore + orderAccuracyScore + acceptanceRateScore;

  console.log('Performance score calculation breakdown', 'PerformanceMetricsAPI', {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const completedReelOrdersWithPhotos = data.CompletedReelOrdersWithPhotos?.aggregate?.count || 0;
    ...
    const onTimeDelivery = calculateCombinedDeliveryTimePerformance(regularOrders, reelOrders);
    ...
    const orderAccuracy =
      assignedOrders > 0 ? Math.round((completedOrdersWithPhotos / assignedOrders) * 100) : 100;

    // Calculate recent performance
    const recentPerformance = {
      last7Days: allRecentOrders.filter((order: any) => order.status === 'delivered').length,
      last30Days: allOrders.filter((order: any) => order.status === 'delivered').length,
    };
    ...
    const performanceMetrics: PerformanceMetrics = {
      ...
      breakdown: {
        deliveryExperience: parseFloat(ratings.avg?.delivery_experience?.toFixed(2)) || 0,
        packagingQuality: parseFloat(ratings.avg?.packaging_quality?.toFixed(2)) || 0,
        professionalism: parseFloat(ratings.avg?.professionalism?.toFixed(2)) || 0,
      },
    };

    // Calculate overall performance score
    logger.info('Performance metrics before calculation', 'PerformanceMetricsAPI', {
      shopperId,
    ...
    performanceMetrics.performanceScore = calculateOverallScore(performanceMetrics);
    ...
    logger.error('Error fetching performance metrics', 'PerformanceMetricsAPI', error);
