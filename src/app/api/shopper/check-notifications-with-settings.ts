  ...
  try {
    ...
    const GET_ACTIVE_ORDERS = gql`
      query GetActiveOrders($user_id: uuid!) {
        Orders(
          where: { shopper_id: { _eq: $user_id }, status: { _in: ["shopping", "delivering"] } }
        ) {
          id
          status
        }
    ...
    const activeOrdersResponse = (await hasuraClient.request(GET_ACTIVE_ORDERS, {
      user_id,
    })) as any;
    const hasActiveOrders =
      (activeOrdersResponse.Orders?.length || 0) + (activeOrdersResponse.reel_orders?.length || 0) >
      0;
