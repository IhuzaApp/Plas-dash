...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    logger.info('Active batches query results', 'ActiveBatchesAPI', {
      ...
      totalOrders:
        regularOrders.length + reelOrders.length + restaurantOrders.length + businessOrders.length,
    });
    ...
    Array.from(combinedOrdersMap.entries()).forEach(([combinedOrderId, orders]) => {
      if (orders.length === 1) {
      ...
      const totalEarnings = orders.reduce(
        (sum, o) => sum + parseFloat(o.service_fee || '0') + parseFloat(o.delivery_fee || '0'),
        0
      );
      const shopNamesArray = Array.from(new Set(orders.map(o => o.Shop.name)));
      ...
      const customerAddresses = Array.from(
        new Set(orders.map(o => `${o.Address.street}, ${o.Address.city}`).filter(Boolean))
      ) as string[];
      ...
      transformedCombinedOrders.push({
        ...
        orderIds: Array.from(new Set([firstOrder.id, ...orders.map(o => o.id)])),
        orderIDs,
      });
    });

    // Transform single orders that had combined_order_id but are alone in their group → regular
    const transformedSingleOrderAsRegular = singleOrderFromCombinedGroup.map(o => ({
      id: o.id,
      OrderID: o.OrderID,
      status: o.status,
    ...
    const transformedBusinessOrders = businessOrders.map(o => {
        ...
        typeof da === 'string'
          ...
          : da && typeof da === 'object'
            ? [(da as any).street, (da as any).city, (da as any).address]
                ...
                .join(', ') || '—'
            : '—';
      const custLat = parseFloat(String(o.latitude ?? (da && (da as any).latitude) ?? 0));
      const custLng = parseFloat(String(o.longitude ?? (da && (da as any).longitude) ?? 0));
      ...
      const deliveryTime =
        o.delivered_time && o.delivered_time.trim() !== ''
          ? o.delivered_time
          : new Date(new Date(o.created_at).getTime() + 2 * 60 * 60 * 1000).toISOString();
      return {
        id: o.id,
        OrderID: o.OrderID != null ? o.OrderID : o.id,
        status: o.status,
        ...
        total: parseFloat(o.total || '0'),
        estimatedEarnings: (
          parseFloat(o.transportation_fee || '0') + parseFloat(o.service_fee || '0')
        ).toFixed(2),
