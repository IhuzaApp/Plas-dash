...
const UPDATE_EMPLOYEE_PRIVILEGES = gql`
  mutation UpdateEmployeePrivileges($id: uuid!, $privillages: jsonb!, $update_on: timestamptz!) {
    update_orgEmployeeRoles(where: {orgEmployeeID: {_eq: $id}}, _set: {privillages: $privillages, update_on: $update_on}) {
      affected_rows
    }
  }
`;
...
export async function POST(request: Request) {
    ...
    try {
        ...
        return NextResponse.json({
            success: true,
            affected_rows: results.reduce((acc, res: any) => acc + (res.update_orgEmployees?.affected_rows || res.update_orgEmployeeRoles?.affected_rows || 0), 0)
        });
