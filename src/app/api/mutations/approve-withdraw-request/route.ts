...
const UPDATE_BUSINESS_WALLET = gql`
  mutation UpdateBusinessWallet($amount: String!, $updated_at: timestamptz!, $business_id: uuid!) {
    ...
    if (isBusiness) {
      ...
      if (currentBalance < amount) {
        return NextResponse.json(
          { error: 'Insufficient business wallet balance' },
          { status: 400 }
        );
      }
      ...
      if (!walletId) {
        return NextResponse.json(
          { error: 'Shopper wallet ID missing on request' },
          { status: 400 }
        );
      }
