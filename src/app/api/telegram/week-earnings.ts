...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const weekStart = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - daysToMonday, 0, 0, 0)
    );
    ...
    const data = await hasuraClient.request<GraphQLResponse>(GET_WEEK_COMPLETED_EARNINGS, {
      shopper_id: userId,
    ...
    return res.status(500).json({
      error: error instanceof Error ? error.message : "Failed to fetch week's completed earnings",
    });
  }
}
