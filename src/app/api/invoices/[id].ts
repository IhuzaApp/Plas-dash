  ...
  try {
    ...
    if (isReelOrder) {
      ...
      transformedInvoice = {
        ...
        reel_title:
          reelOrderDetails?.Reel?.title || invoice.invoice_items?.[0]?.description || 'Reel Order',
        reel_description:
          reelOrderDetails?.Reel?.description || invoice.invoice_items?.[0]?.description || '',
        delivery_photo_url:
          reelOrderDetails?.delivery_photo_url || invoice.Order?.delivery_photo_url,
        // Restaurant details
