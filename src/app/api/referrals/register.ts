...
const CHECK_DUPLICATE_REFERRAL = gql`
  query CheckDuplicateReferral($user_id: uuid!, $phone: String!, $deviceFingerprint: String!) {
    Referral_window(
      where: {
        _or: [
          { user_id: { _eq: $user_id } }
 ...
 */
async function generateUniqueReferralCode(name: string, maxAttempts: number = 10): Promise<string> {
