...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const data = await hasuraClient.request<ShopRatingsResponse>(GET_SHOP_RATINGS);
    ...
    const ratings = Object.keys(shopRatingsMap).map(shop_id => ({
      shop_id,
      averageRating: shopRatingsMap[shop_id].totalRating / shopRatingsMap[shop_id].count,
