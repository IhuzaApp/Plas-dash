    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Tax Reports</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">Generate, preview, and export your official tax documents.</p>
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                    <Button variant="outline" className="gap-2 border-slate-300 dark:border-slate-700 h-10 hover:bg-slate-50 dark:hover:bg-slate-800 dark:text-slate-200 transition-colors" onClick={handlePrint}>
                        <Printer className="h-4 w-4" /> Print
                    </Button>
                    <Button variant="outline" className="gap-2 border-slate-300 dark:border-slate-700 h-10 text-green-700 dark:text-green-400 hover:bg-green-50 dark:hover:bg-green-900/30 hover:border-green-300 transition-colors">
                ...
                <CardContent className="p-5">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="space-y-1.5">
                            <Label className="text-xs text-slate-500 dark:text-slate-400 font-medium">Tax Year</Label>
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-xs text-slate-500 dark:text-slate-400 font-medium">Quarter</Label>
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-xs text-slate-500 dark:text-slate-400 font-medium">Jurisdiction</Label>
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-xs text-slate-500 dark:text-slate-400 font-medium">Business Entity</Label>
                        ...
                        <Card key={r.id}
                            ...
                            <CardContent className="p-5 flex flex-col gap-4">
                                <div className="flex items-center justify-between">
                                    <div className={`p-2.5 rounded-xl bg-${r.color}-50 dark:bg-${r.color}-900/30 text-${r.color}-600 dark:text-${r.color}-400`}>
                                        <Icon className="h-5 w-5" />
                                    </div>
                                    <Badge variant="outline" className="text-[10px] font-semibold text-green-700 dark:text-green-400 border-green-200 dark:border-green-700 bg-green-50 dark:bg-green-900/30">
                                        {r.status}
                                    </Badge>
                                </div>
                                <div>
                                    <p className="font-semibold text-slate-900 dark:text-slate-100 text-sm leading-tight">{r.title}</p>
                                ...
                                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                    ...
                                    <Button variant="ghost" size="sm" className="h-7 px-2 text-xs gap-1 text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors">
            ...
            <div>
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-lg shadow-slate-900/5 overflow-hidden print:shadow-none print:border-none">
                    ...
                    <div className="bg-slate-900 text-white px-10 py-8 print:bg-white print:text-slate-900">
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                            <div>
                                ...
                                <p className="text-slate-400 text-sm mt-1 print:text-slate-600">{COMPANY.address}</p>
                            </div>
                            <div className="text-right shrink-0">
                                <p className="text-xs uppercase tracking-widest text-slate-400 mb-1 print:text-slate-500">Tax Return</p>
                                <p className="text-xl font-mono font-bold text-blue-400 print:text-blue-700">FY {year}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-10 py-8 space-y-8">
                        ...
                        {/* Company Information */}
                        <section>
                            <h3 className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-4">Company Information</h3>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {[
                                    ...
                                    <div key={item.label} className="bg-slate-50 dark:bg-slate-800 rounded-xl p-4">
                                        <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">{item.label}</p>
                        ...
                        {/* Revenue Summary */}
                        <section>
                            <h3 className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-4">Revenue Summary</h3>
                            <table className="w-full text-sm">
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {[
                                        { label: "Gross Revenue", value: FINANCIALS.gross_revenue },
                                        { label: "Cost of Goods Sold (COGS)", value: -FINANCIALS.cost_of_goods, negative: true },
                                    ].map((row) => (
                                        <tr key={row.label}>
                                            <td className="py-2.5 text-slate-600 dark:text-slate-400">{row.label}</td>
                                            <td className={`py-2.5 text-right font-mono font-semibold ${row.negative ? "text-red-600 dark:text-red-400" : "text-slate-900 dark:text-slate-100"}`}>
                                                {row.negative ? `(${fmt(Math.abs(row.value))})` : fmt(row.value)}
                                            </td>
                                        </tr>
                                    ))}
                                    <tr className="font-bold bg-slate-50 dark:bg-slate-800 rounded">
                                        <td className="py-3 px-2 text-slate-900 dark:text-slate-100 rounded-l-lg">Gross Profit</td>
                                        <td className="py-3 px-2 text-right font-mono text-blue-700 dark:text-blue-400 rounded-r-lg">{fmt(FINANCIALS.gross_profit)}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </section>
                        ...
                        {/* Deduction Summary */}
                        <section>
                            <h3 className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-4">Deduction Summary</h3>
                            <table className="w-full text-sm">
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {[
                                        ...
                                        <tr key={row.label}>
                                            <td className="py-2.5 text-slate-600 dark:text-slate-400">{row.label}</td>
                                            <td className="py-2.5 text-right font-mono text-slate-700 dark:text-slate-300">{fmt(row.value)}</td>
                                        </tr>
                                    ))}
                                    <tr className="font-bold bg-slate-50 dark:bg-slate-800">
                                        <td className="py-3 px-2 text-slate-900 dark:text-slate-100 rounded-l-lg">Total Deductions</td>
                        ...
                        {/* Tax Calculation */}
                        <section>
                            <h3 className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-4">Tax Calculation</h3>
                            <div className="bg-slate-900 dark:bg-slate-950 rounded-2xl p-6 text-white">
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                                    {[
                                        { label: "Taxable Income", value: fmt(FINANCIALS.taxable_income), highlight: false },
                                        { label: "Applied Tax Rate", value: `${(FINANCIALS.tax_rate * 100).toFixed(1)}%`, highlight: false },
                                        { label: "Effective Rate", value: `${FINANCIALS.effective_rate.toFixed(2)}%`, highlight: false },
                                        { label: "Estimated Tax Due", value: fmt(FINANCIALS.estimated_tax), highlight: true },
                                    ].map((item) => (
                                        <div key={item.label} className={`p-4 rounded-xl ${item.highlight ? "bg-blue-500/20 border border-blue-500/30" : "bg-white/5"}`}>
                                            <p className="text-[10px] uppercase tracking-widest text-slate-400 mb-2">{item.label}</p>
                                            <p className={`text-xl font-bold font-mono ${item.highlight ? "text-blue-300" : "text-white"}`}>{item.value}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        {/* Signature Section */}
                        <section>
                            <h3 className="text-xs uppercase tracking-widest font-bold text-slate-400 dark:text-slate-500 mb-6">Authorization & Signature</h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {[
                                    { role: "Authorized Officer / Director", name: "Jane Doe", title: "Chief Financial Officer" },
                                    { role: "Tax Preparer / CPA", name: "John Smith, CPA", title: "External Tax Advisor" },
                                ].map((sig) => (
                                    <div key={sig.role} className="space-y-4">
                                        <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">{sig.role}</p>
                                        ...
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div>
                                                <p className="font-semibold text-slate-900 dark:text-slate-100">{sig.name}</p>
                                                <p className="text-xs text-slate-500 dark:text-slate-400">{sig.title}</p>
                                                <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">Date: ___________________</p>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="mt-8 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-4 text-xs text-amber-800 dark:text-amber-300">
                                <strong>Disclaimer:</strong> This report is generated from the PlasDash Tax Engine using internally provided financial data.
                                It is intended for review purposes only and does not constitute a filed tax return.
                                Please review with a qualified tax professional before submission.
                            </div>
                        </section>
