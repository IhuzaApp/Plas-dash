...
const SUGGESTIONS = [
    {
        ...
        description: "Switch to MACRS accelerated depreciation for hardware acquired in Q3 to increase immediate deductible expenses.",
        savings: 12000,
        impactLevel: "high",
        icon: Zap,
        color: "blue"
    },
    {
        ...
        description: "Pre-pay January rent and software annual renewals in December to reduce current year taxable income.",
        savings: 8500,
        impactLevel: "medium",
        icon: TrendingDown,
        color: "amber"
    },
    {
        ...
        description: "File Form 6765 for documented software development engineering hours spent on the new analytics engine.",
        savings: 15200,
        impactLevel: "high",
        icon: Target,
        color: "purple"
...
export default function TaxOptimizationPage() {
    ...
    const activeSavings = SUGGESTIONS.filter(s => activeSimulations[s.id]).reduce((sum, s) => sum + s.savings, 0);
    ...
    const additionalScore = totalPotentialSavings > 0 ? (activeSavings / totalPotentialSavings) * (maxPossibleScore - baseScore) : 0;
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-50">Tax Optimization</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">AI-driven suggestions to maximize deductions and reduce your tax liability.</p>
                </div>
            </div>
                    ...
                    <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                            <div className="space-y-2">
                                ...
                                <div className="text-3xl font-bold text-slate-900 dark:text-slate-50">{missedDeductionsCount}</div>
                            </div>
                            <div className="p-3 bg-amber-100 dark:bg-amber-900/30 rounded-xl text-amber-600 dark:text-amber-400">
                                <Lightbulb className="h-5 w-5" />
                            </div>
                        </div>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-4 leading-relaxed">Review suggestions below to capture these opportunities before year-end.</p>
                    </CardContent>
                </Card>
                    ...
                    <CardContent className="p-6">
                        <div className="flex justify-between items-start">
                            <div className="space-y-2">
                                ...
                                <div className="text-3xl font-bold text-slate-900 dark:text-slate-50">{formatCurrency(totalPotentialSavings)}</div>
                            </div>
                            <div className="p-3 bg-green-100 dark:bg-green-900/30 rounded-xl text-green-600 dark:text-green-400">
                                <Calculator className="h-5 w-5" />
                            </div>
                        </div>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-4 leading-relaxed">Total identified savings across all active intelligence modules.</p>
                    </CardContent>
                </Card>
                        ...
                        <div className="flex justify-between items-start">
                            <div className="space-y-2 w-full">
                                ...
                                <div className="flex items-end gap-3 mb-2">
                                    <div className="text-3xl font-bold text-slate-900 dark:text-slate-50">{currentScore}%</div>
                                    <span className={`text-sm font-medium mb-1 ${currentScore > 80 ? 'text-green-600 dark:text-green-400' : currentScore > 50 ? 'text-amber-500' : 'text-red-500'}`}>
                                        {currentScore > 80 ? "Excellent" : currentScore > 50 ? "Needs Review" : "Action Required"}
                                    </span>
                                </div>
                                <Progress value={currentScore} className="h-2 w-full bg-slate-100 dark:bg-slate-800" indicatorClassName={currentScore > 80 ? 'bg-green-500' : currentScore > 50 ? 'bg-amber-500' : 'bg-red-500'} />
                            </div>
                        </div>
                    </CardContent>
                </Card>
                                ...
                                <Card key={suggestion.id}
                                    ...
                                    <CardContent className="p-6 h-full flex flex-col">
                                        <div className="flex items-start gap-4 mb-4">
                                            <div className={`p-3 rounded-2xl shrink-0 ${isActive ? `bg-${suggestion.color}-500 text-white` : `bg-${suggestion.color}-50 dark:bg-${suggestion.color}-900/30 text-${suggestion.color}-600 dark:text-${suggestion.color}-400`}`}>
                                                <Icon className="h-6 w-6" />
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-slate-900 dark:text-slate-100 leading-tight mb-1">{suggestion.title}</h4>
                                        ...
                                        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed flex-1 mb-6">{suggestion.description}</p>
                                        <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                            <div>
                                                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wider mb-0.5">Est. Savings</p>
                                                <p className="font-bold text-green-600 dark:text-green-400 text-lg">{formatCurrency(suggestion.savings)}</p>
                                            </div>
                                            <Button
                                            ...
                                            >
                                                {isActive ? <><CheckCircle2 className="h-4 w-4" /> Applied</> : <><ArrowRight className="h-4 w-4" /> Simulate</>}
                                            </Button>
                                        </div>
                                    </CardContent>
                                </Card>
                ...
                <div className="xl:col-span-1 space-y-6">
                    ...
                    <Card className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-lg sticky top-24">
                        <CardHeader className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 pb-4">
                            <CardTitle className="text-base text-slate-800 dark:text-slate-100">Tax Liability Comparison</CardTitle>
                            <CardDescription className="dark:text-slate-400">Current projection vs. applied strategies</CardDescription>
                        </CardHeader>
                        <CardContent className="p-6">
                            <div className="flex justify-between items-end mb-6">
                                <div>
                                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Optimized Tax</p>
                                    <p className="text-3xl font-bold text-slate-900 dark:text-slate-50">{formatCurrency(optimizedTaxDue)}</p>
                                </div>
                                {activeSavings > 0 && (
                            ...
                            <div className="h-[220px] w-full mt-4">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={comparisonData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                        ...
                                        <YAxis axisLine={false} tickLine={false} tick={{ fill: "#94a3b8", fontSize: 12 }} tickFormatter={formatCompact} domain={[0, Math.ceil(CURRENT_TAX_DUE * 1.1)]} />
                                        <Tooltip cursor={{ fill: "transparent" }} contentStyle={{ borderRadius: "12px", border: "1px solid #e2e8f0" }} formatter={(value: number) => [formatCurrency(value), "Estimated Tax"]} />
                        ...
                        <CardFooter className="bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 p-4">
                            <div className="w-full space-y-2">
                                <div className="flex justify-between text-sm">
                                    ...
                                    <span className="font-medium text-slate-900 dark:text-slate-100">{formatCurrency(CURRENT_TAX_DUE)}</span>
                                </div>
                                <div className="flex justify-between text-sm">
                                    <span className="text-slate-500 dark:text-slate-400">Simulated Savings:</span>
                                    <span className="font-medium text-green-600 dark:text-green-400">-{formatCurrency(activeSavings)}</span>
                                </div>
                            </div>
                        </CardFooter>
                    </Card>
