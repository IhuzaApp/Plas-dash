...
const TAX_BREAKDOWN = [
  {
    ...
    tooltip:
      'Federal corporate income tax at 21% (Tax Cuts and Jobs Act, 2017). Applied to taxable income after all deductions.',
  },
  {
    id: 'state',
    label: 'State Tax (CA)',
    ...
    tooltip:
      'California state corporate income tax at 8.84% — one of the highest in the US. Applied to California-apportioned income.',
  },
  {
    id: 'local',
    ...
    tooltip:
      'San Francisco local business tax at ~1.5% of gross receipts. Rate varies by business category.',
  },
];
...
const renderPieLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent }: any) => {
  ...
  return (
    <text
      x={cx + radius * Math.cos(-midAngle * R)}
      y={cy + radius * Math.sin(-midAngle * R)}
      fill="white"
      textAnchor="middle"
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 dark:text-slate-400">
              Tax liability automatically computed from your last import.
            </p>
          </div>
          <div className="flex items-center gap-2 flex-wrap shrink-0">
            ...
            >
              <Link href="/tax/import">
                <SlidersHorizontal className="h-4 w-4" /> View Adjustments
              </Link>
            </Button>
            <Button
              variant="outline"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          {[
            ...
            return (
              <Card
                key={kpi.label}
                className={`relative overflow-hidden rounded-2xl border-none shadow-sm
              ...
              >
                <CardContent className="p-5 relative">
                  <div className="flex justify-between items-start mb-4">
                    <p
                      className={`text-sm font-medium ${kpi.darkCard ? 'text-slate-400' : 'text-slate-500 dark:text-slate-400'}`}
                    >
                      {kpi.label}
                    </p>
                  ...
                  <p
                    className={`text-2xl font-black tracking-tight ${kpi.darkCard ? 'text-white' : 'text-slate-900 dark:text-slate-50'}`}
                  >
                    {kpi.value}
                  </p>
                  {kpi.darkCard && (
                    <p className="text-xs text-slate-400 mt-2">
                      Effective rate: {EFFECTIVE_RATE.toFixed(1)}%
                    </p>
                  )}
                </CardContent>
              </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Card className="lg:col-span-2 border border-slate-100 dark:border-slate-800 shadow-sm rounded-2xl bg-white dark:bg-slate-900">
            <CardHeader className="pb-2">
              <CardTitle className="text-base text-slate-800 dark:text-slate-100">
                Monthly Revenue vs Tax Liability
              </CardTitle>
              <CardDescription className="dark:text-slate-400">
                Full-year trend from imported data
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="border border-slate-100 dark:border-slate-800 shadow-sm rounded-2xl bg-white dark:bg-slate-900">
            <CardHeader className="pb-2">
              ...
              <CardDescription className="dark:text-slate-400">
                Share of total deductible expenses
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card className="lg:col-span-2 border border-slate-100 dark:border-slate-800 shadow-sm rounded-2xl overflow-hidden bg-white dark:bg-slate-900">
            <CardHeader className="pb-2 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/50">
              <CardTitle className="text-base text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <Calculator className="h-4 w-4 text-blue-600 dark:text-blue-400" /> Tax Jurisdiction
                Breakdown
              </CardTitle>
              <CardDescription className="dark:text-slate-400">
                Federal + State + Local liability totals
              </CardDescription>
            </CardHeader>
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800">
                {TAX_BREAKDOWN.map(tax => {
                  ...
                  return (
                    <div
                      key={tax.id}
                      className="flex items-center justify-between px-6 py-4 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                            Rate: <span className="font-semibold">{tax.rate}</span>
                          </p>
                        </div>
                      </div>
